...
class ZeebeAPI {
   ...
   * Camunda REST client.
   *
   * @param {{ endpoint: import("./endpoints").Endpoint, resourceType: string }} config
   *
   * @returns {Promise<{ success: boolean, response?: object, reason?: string }>}
   */
   ...
   *
   * @param {{ endpoint: import("./endpoints").Endpoint, filter: object }} config
   *
  ...
  async getClusterVariable(config) {
    ...
    try {
      ...
      const response = await camundaRestClient.callApiEndpoint({
        method: 'GET',
        urlPath: `cluster-variables/global/${ name }`
      });
