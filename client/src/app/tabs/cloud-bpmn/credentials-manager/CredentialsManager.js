...
// Reasons the credential chooser cannot query/manage configurations, with a
// user-facing message each.
...
// Search filter selecting credential cluster variables; metadata filters require
// operator objects (`$eq`), bare values are rejected pre-8.10.
...
export default class CredentialsManager extends PureComponent {
  ...
  async createCredential({ endpoint, event, name, value, metadata }) {
    const result = await this.props.zeebeApi.createClusterVariable({ endpoint }, { name, value, metadata });
